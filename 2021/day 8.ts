...
function getSide(bigStr, lilStr) {
  let bigArr = new Set(bigStr.split(""));
  let lilArr = new Set(lilStr.split(""));
  return difference(bigArr, lilArr);
}

// console.log(getSide("gcf", "fg")); OK

// format input to:
// [[inputStr, outputLineStr]]
function parseInput(input) {
  let parsedInput;

  let rows = input.split("\n");

  parsedInput = rows.map((row) => row.split("|"));
  return parsedInput;
}

function countUniqueDigitsInOutput(parsedInput) {
  let total = 0;

  console.log("test");

  for (let item of parsedInput) {
    let [inputStr, outputStr] = item;
    let currMap = {};
    let reverseMap = {};

    let outputArr = outputStr
      .split(" ")
    ...
    function checkMap(thing, index) {
      // add unique values to currMap
      // 1
      if (thing.length == 2) {
        currMap[1] = thing;
        reverseMap[thing] = 1;
        ...
        reverseMap[thing] = 4;
        // 8
      } else if (thing.length == 7) {
        currMap[8] = thing;
        reverseMap[thing] = 8;
      } else if (thing.length == 6 && currMap[1] && currMap[4]) {
        // 6 will be
        console.log(thing);
        console.log(currMap[1]);
        if (getSide(thing, currMap[1]).size == 1) {
          currMap[6] = thing;
          reverseMap[thing] = 6;
          // 0 will be
        } else if (currMap[4] && getSide(thing, currMap[4]).size == 1) {
          currMap[0] = thing;
          reverseMap[thing] = 0;
          // 9 will be
        } else {
      ...
      } else if (thing.length == 5 && 6 in currMap && currMap[5]) {
        // 5
        if (currMap[6] && getSide(thing, currMap[6]).size == 1) {
          currMap[5] = thing;
          reverseMap[thing] = 5;
          // 3
        } else if (currMap[5] && getSide(thing, currMap[5]).size == 1) {
          currMap[3] = thing;
          reverseMap[thing] = 3;
        } else {
          currMap[2] = thing;
    ...
    while (i < 1000) {
      inputArr.forEach((str, index) => {
        ...
        // console.log(str);
        // console.log(!(outputArr[0] in reverseMap));
        // console.log(!(outputArr[1] in reverseMap));
        // console.log(!(outputArr[2] in reverseMap));
        // console.log(!(outputArr[3] in reverseMap));
        if (!(str in reverseMap)) {
          checkMap(str, index);
        }
      });
      // inputArr = inputArr.filter((x) => !(x in reverseMap));
...
// part 1 testing
// console.log(parseInput(testInput)); OK
// console.log(countUniqueDigitsInOutput(parseInput(testInput))); OK

// Part 1 Answer
// console.log(countUniqueDigitsInOutput(parseInput(input))); OK

// Part 2 testing
console.log(countUniqueDigitsInOutput(parseInput(testInput)));
